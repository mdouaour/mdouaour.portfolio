  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3">
        <h3 className="text-text font-semibold text-lg group-hover:text-text-accent transition-colors leading-snug">
          {title}
        </h3>
        <span
        ...
        <ul className="space-y-1">
          {features.map((feat) => (
            <li key={feat} className="text-text-muted text-xs flex items-start gap-1.5">
              <span className="text-text-accent flex-shrink-0">▹</span>
              {feat}
            </li>
          ))}
        </ul>
      ...
      <div className="flex flex-wrap gap-2">
        {project.tags.map((tag) => (
          <span
            key={tag}
            className="text-xs text-text-accent bg-violet-500/10 border border-violet-500/20 px-2 py-0.5 rounded-full"
          >
            {tag}
          </span>
        ))}
      ...
      <div className="flex flex-wrap gap-4 mt-auto pt-2 border-t border-border">
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="inline-flex items-center gap-1.5 text-sm text-text-muted hover:text-text-accent transition-colors"
        ...
        {project.demo && (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center gap-1.5 text-sm text-text-accent hover:text-text-accent transition-colors"
